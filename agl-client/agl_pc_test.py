#!/usr/bin/env python3
"""Stdlib rosbridge client for AGL: subscribe to the RAW LiDAR point cloud.
Usage: python3 agl_pc_test.py 10.0.2.2 9090 /carla/lidar"""
...
def main():
    ...
    topic = sys.argv[3] if len(sys.argv) > 3 else "/carla/lidar"
    print("[agl] connecting to ws://%s:%d topic=%s" % (host, port, topic))
    s = ws_connect(host, port)
    print("[agl] handshake OK, subscribing to point cloud...")
    ...
    while True:
        ...
        m = d.get("msg", {})
        w = m.get("width", 0); ps = m.get("point_step", 16)
        data = m.get("data", "")
        ...
        if len(raw) >= 12:
            ...
            first = "  first pt=(%.2f,%.2f,%.2f)" % (x, y, z)
        print("[agl] #%d %s  %d points, %d bytes%s" % (count, topic, w, len(raw), first))
